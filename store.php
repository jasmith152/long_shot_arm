...
if (!empty($_GET['cat'])) { $_SESSION['cat'] = $_GET['cat']; } else { $_SESSION['cat'] = ''; }
if (!empty($_POST['cat'])) { $_SESSION['cat'] = $_POST['cat']; } else { $_SESSION['cat'] = ''; }
if (!empty($_GET['subcat'])) { $_SESSION['subcat'] = $_GET['subcat']; } else { $_SESSION['subcat'] = ''; }
if (!empty($_POST['subcat'])) { $_SESSION['subcat'] = $_POST['subcat']; } else { $_SESSION['subcat'] = ''; }
if (!empty($_GET['brand'])) { $_SESSION['brand'] = $_GET['brand']; }
if (!empty($_POST['brand'])) { $_SESSION['brand'] = $_POST['brand']; }
if (!empty($_GET['spot'])) { $_SESSION['spot'] = $_GET['spot']; }
if (!empty($_POST['spot'])) { $_SESSION['spot'] = $_POST['spot']; }
...
$page_title = "Order Custom Guns Online, Guns, Rifles, Shotguns, Handguns for Sale in 
Marion County Florida.";
$page_descr = "Long Shot Arms LLC is your source for custom rifles, home defense shotguns, 
and new and used weapons for sale online.";
// Include site header
...
/*
// Show Product Categories ... wh 20120716
echo "<table width='80%' border='1' cellpadding='0' cellspacing='0' align='center'>";
echo "	<tr>";
$sql_cat="SELECT DISTINCT category FROM `tbl_products`";
$result_cat = mysql_query($sql_cat);
$fetch_cat=mysql_fetch_array($result_cat);
$i=1;
while($fetch_cat){
	if ($i % 2){
		echo "		<td name='storelinksR' style='font: 12pt arial; color: white; text-align:left; padding-left:25px;' width='50%'>".$category."</td>\n";
		// New row after right column 
		echo "	</tr>\n	<tr>\n"; 
		}
	else {
		echo "		<td name='storelinksL' style='font: 12pt arial; color: white; text-align:left; padding-left:25px;' width='50%'>".$category."</td>\n";
		}	
	$i++;
	}
echo "</table>";
...
mysql_close($dbcnx);
?>
<div align="center"><a href="http://www.davidsonsinc.com/consumers/subsites/dealer_home.asp?dealer_id=796177" target="_blank"><img src="images/GunGenieLogo.jpg" width="179" height="194" /></a><br />
  <br />
   <br />
<div id="white-box2"><br />
<a href="category.php?cat=Custom Pistols"><img src="images/headers/custom-pistols.png" alt="Custom Pistols" width="324" height="54" /></a><br />
<a href="category.php?cat=Custom Rifles"><img src="images/headers/custom-rifles.png" alt="Custom Rifles" width="324" height="61" /></a><br />
<a href="category.php?cat=Custom Shotguns"><img src="images/headers/custom-shotguns.png" alt="Custom Shotguns" width="324" height="61" /></a><br />
<a href="category.php?cat=Custom AR15 Rifles"><img src="images/headers/custom-ar-rifles.png" alt="Custom AR15 Rifles" width="324" height="61"  /></a><br />
     <hr size="1" noshade="true" />
     <a href="view_cart.php">View Cart</a><br />
<?php
if (empty($_SESSION['UNAME'])) {
   echo "     <a href='view_cart.php?action=login'>Customer Login</a><br />\n";
} else {
